...
def needed_rows(
    df_to_look_for_indexes: pd.DataFrame,
    criteries: list
) -> pd.DataFrame:
    """ Finds multiple rows by given criterias

    Args:
        df_to_look_for_indexes (pd.DataFrame): DataFrame in which we want to find rows\n
    ...
    for criteria in range(len(criteries)):
        print(criteria)
        df_to_look_for_indexes = df_to_look_for_indexes[
            (df_to_look_for_indexes.loc[:,criteries[criteria]["column"]] == \
                criteries[criteria]["criteria"])
        ].copy()
    return df_to_look_for_indexes